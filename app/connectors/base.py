...
list of `CanonicalMarket`. Each connector owns its own error handling: a failure
in one venue must NEVER take down the other, so `fetch()` should catch, log, and
return whatever it has (often an empty list).
"""
...
class Connector(Protocol):
    ...
    async def fetch(self) -> list[CanonicalMarket]:
        """Return normalized markets for this venue. Never raises for venue errors."""
        ...
